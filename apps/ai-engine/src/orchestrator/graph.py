...
class SMSAAIAssistantOrchestratorGraph:
    ...
    async def run_stream(self, initial_state: Dict[str, Any]):
        """
        Execute the orchestrator workflow with streaming support.

        Yields chunks of content as they're generated by the LLM.

        Args:
            initial_state: Initial state dict with message, conversation_id, etc.

        Yields:
            Dict chunks with 'type', 'content', 'agent', 'metadata'
        """
        ...
        intent_result = await self._classify_intent_node(state)
        state.intent = intent_result["intent"]
        state.intent_confidence = intent_result["intent_confidence"]
        state.parameters = intent_result.get("parameters", {})

        # Step 2: Assemble context
        context_result = await self._assemble_context_node(state)
        state.conversation_history = context_result["conversation_history"]
        state.file_context = context_result["file_context"]
        state.semantic_context = context_result["semantic_context"]

        # Step 3: Route to agent with streaming
        # Extract AWB from file context if available (for tracking agent)
        parameters = state.parameters.copy()
        if state.file_context:
            extracted_data = state.file_context.get("extracted_data", {})
        ...
        full_content = ""
        if state.intent == Intent.TRACKING:
            async for chunk in self._tracking_agent.run_stream(context):
                full_content += chunk.get("content", "")
                if chunk.get("metadata"):
                    metadata.update(chunk["metadata"])
                yield chunk
        elif state.intent == Intent.RATES:
            async for chunk in self._rates_agent.run_stream(context):
                full_content += chunk.get("content", "")
                if chunk.get("metadata"):
                    metadata.update(chunk["metadata"])
                yield chunk
        elif state.intent == Intent.LOCATIONS:
            async for chunk in self._retail_agent.run_stream(context):
                full_content += chunk.get("content", "")
                if chunk.get("metadata"):
                    metadata.update(chunk["metadata"])
                yield chunk
        elif state.intent == Intent.FAQ:
            async for chunk in self._faq_agent.run_stream(context):
                full_content += chunk.get("content", "")
                if chunk.get("metadata"):
                    metadata.update(chunk["metadata"])
                yield chunk
        ...
        # Step 4: Save conversation to MongoDB (async, don't block streaming)
        try:
            if state.conversation_id and state.conversation_id != "default":
                user_id = state.user_id or "anonymous"
            ...
            logger.warning("mongodb_save_failed", error=str(db_error))
    ...
    async def _save_conversation_async(
        ...
        """Save conversation to MongoDB asynchronously (background task)."""
        try:
